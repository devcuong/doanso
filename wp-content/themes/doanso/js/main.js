...
// left menu
var menuIconImage = $("ul.leftMenuFixed > li > a > h4 > img");
var menuUl = $("ul.leftMenuFixed");
var menuUl2 = $("ul.leftMenuFixed > li > ul");
var count = 0;
menuIconImage.click(function(e){
	e.preventDefault();
	if(count == 0){
		count++;
		menuUl2.css('display','block');
		menuUl.animate({left: '-40px'},"slow", function(){ menuIconImage.css('animation-play-state', 'paused');});
	}
	else{
		count--;
		menuUl.animate({left: '-249px'},"slow", function(){ menuIconImage.css('animation-play-state', 'running');});
		menuUl2.animate({display: 'none'});
	}
});
//tabbed widget
...
//fix sidebar when croll
 /*$(window).bind('scroll', function() {
   var navHeight = $( window ).height();
	var navFoot=$( ".main-col" ).height();
	 //alert(navHeight+"và "+navFoot);
		 ...
		 if($(window).scrollTop()>navFoot){
			  //$('#left').removeClass('fixed');
			  $('.main-sidebar').addClass('notfixed');
		 }
		 else{
			 $('.main-sidebar').removeClass('notfixed');
		 }
	});*/
// videos
